...
const createServer = () => {
  ...
  function foodHandler(request,response) {
    let food = ['Pizza','Tacos','Ice Cream']
...
const count = (target, input) => {
  ...
  if(x.length === 0){
    ...
    x = x.map(i=> i.reduce(add,0));
    return x.reduce((total,num) => total + num)
  }
};
...
let findMaleAndFemale = (data) => {
  let filtered = data.filter(i => i.gender === 'male' || i.gender === 'female');
  let persons = filtered.map(i => i.name);
...
let findShortest = (data) => {
  ...
  let height = heights.reduce(
    function reducer(acc,num) { return acc + parseInt(num)},0
  )
  let shorty = heights.reduce(
  ...
  let x = data.filter(i => {
    if(parseInt(i.height) === shorty){
      return i.name
    }
  })
  return x[0].name.toString()
};
